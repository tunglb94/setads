...
def gather_deep_funnel_metrics(ad_id: str) -> dict:
    ...
    # If split data not yet synced, fall back to combined conversions for calculations.
    # Keep meta_messages/meta_comments at 0 so frontend knows no real split exists.
    has_split_data = meta_messages > 0 or meta_comments > 0
    if has_split_data:
        # True CPL uses inbox messages only — comments don't qualify leads
        meta_convs = meta_messages
    else:
        meta_convs = int(spend_agg.get("total_conversions") or 0)

    # ── Quality signals ──────────────────────────────────────────────────────────
    # Prefer conversations attributed to this ad via webhook referral.
    # Fall back to global quality rates when polling-only data (no referral_ad_id).
    attributed = Conversation.objects.filter(referral_ad_id=ad_id)
    has_attribution = attributed.exists()

    if has_attribution:
        total_convs = attributed.count()
        ...
        # No webhook attribution yet — use Meta's count for funnel sizing,
        # apply quality rates from active-page inbox as estimates.
        total_convs = meta_convs
    ...
    cost_per_message = round(total_spend / total_convs) if total_convs > 0 else 0
