...
from .services import sync_insights_to_db, sync_ads_to_db, MetaAPIError
...
def sync_all_accounts(self, days: int = 3):
    ...
    for account in accounts:
        try:
            ...
            total += count
            ad_count = sync_ads_to_db(account, days=days)
            total += ad_count
            logger.info("Synced %d adset + %d ad rows for account %s", count, ad_count, account.account_id)
