...
def sync_insights_to_db(account: AdAccount, days: int = 3) -> int:
    ...
    for row in raw_rows:
        ...
        # Sửa lỗi lấy dữ liệu:
        # Trong Meta API, comment có action_type là 'comment'
        # Inbox có action_type là 'onsite_conversion.messaging_conversation_started_7d'
        comments   = _extract_action_value(actions, "comment")
        messages   = _extract_action_value(actions, "onsite_conversion.messaging_conversation_started_7d")
        
        # Nếu tài khoản của bạn chạy tin nhắn nhưng Meta lại log dưới dạng 'omni_messages' hoặc 'messages'
        if messages == 0:
            messages = _extract_action_value(actions, "messages") or _extract_action_value(actions, "omni_messages")
...
def sync_ads_to_db(account: AdAccount, days: int = 3) -> int:
    ...
    for row in results:
        ...
        # Sửa ở đây tương tự như hàm sync_insights_to_db ở trên
        comments   = _extract_action_value(actions, "comment")
        messages   = _extract_action_value(actions, "onsite_conversion.messaging_conversation_started_7d")
